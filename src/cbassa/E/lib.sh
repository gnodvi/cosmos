...
for TRUE_FILE in ${DIR_TST}/*               # цикл по всем файлам в указанной директории
  do
    ...
    LL=`echo "$T_FILE" | awk 'BEGIN{FS=$METKA} {print $1}'` # отделяем метку уровня    
    ...
    if [ "$LL" != "$LEVEL" ]
    then
        #echo "continue LL= $LL   LEVEL= $LEVEL"
        continue
    fi
    ...
    CALC_FILE=$TMP_DIR/CALC_FILE   # имя временного тестового файла 
    DIFF_FILE=$TMP_DIR/DIFF_FILE   # имя временного файла сравнения 
    ...
    CALC_FILE_SED=$TMP_DIR/CALC_FILE_SED
    TRUE_FILE_SED=$TMP_DIR/DIFF_FILE_SED 
