...
Initialize ()
{
  ...
  for (i = 0; i < 10; i++)
  {
    ...
    if (rand_test > 1 || rand_test < 0)
    {
      ...
      #ifdef USE_RANDOM
      fprintf( stderr, 
              "It is also possible that random() is not returning 31 bit integers?\n" );
      #endif	
      exit( 1 );
    }
  }
    ...
    star_disp_type *sd;
{
  ...
  case KeyPress:
  {
    ...
    if (buf[0]=='q' || buf[0]=='Q')   stop = TRUE;
    if( buf[0]=='d' || buf[0]=='D' )   add = TRUE;

    if ( buf[0]=='+')   add_star = TRUE;
    if ( buf[0]=='-')   del_star = TRUE;

    if (buf[0]=='e' || buf[0]=='E')      erase = TRUE;
    if (buf[0]=='n' || buf[0]=='N')  new_start = TRUE;
    if (buf[0]=='p' || buf[0]=='P') pause_updt = TRUE;
...
//------------------------------------------------------------------------------
void
Parse_Arguments (int argc, char  **argv, char **geometry, 
                 int *num_test, int *y_test_num)
  //int     argc;
  //char    **argv;
  //char    **geometry;
{
  ...
  while( (option = getopt (argc, argv, "y:s:hrt:d:g:D:b:c:C:vRMa:m:B:l:T:") )
        != EOF)
  {
  switch( option )
  {
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     case 'y': //  это я ввел свободную переменную
       *y_test_num = atoi (optarg);
       break;
...
main (int argc, char **argv)
{
  ...
  printf ("\n");
  printf ("XStars: .................................... \n");
  printf ("XStars: timeout = %d \n", timeout);
  printf ("\n");

  switch (y_num_test) {
    
  case 0:  // xstar -y0
    fprintf (stderr, "y_num_test = 0 \n\n");

    //Create_Window (geometry);
    Animate (s_num_test, 20 /* my_count_max */, 1 /* DEBUG_var */, 0 /* is_X */);
  ...
  default: // -1
    ...
    if (! timeout)
    {
      ...
      Create_Window (geometry);
      Animate (s_num_test, my_count_max, 0 /* DEBUG_var */, is_X);
