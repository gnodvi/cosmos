...
//------------------------------------------------------------------------------
void
Animate (int num_test, int my_count_max, int DEBUG_var /* 1 - не рандомизированные тесты*/,
         int is_X)
{


  // надо все-таки вставить свой счетчик для тестирования!!
  // 
  int my_count = 0;
  ...
  sys_param_type    sp;
  star_disp_type    sd, *p_sd = &sd;

  int		    color_skip = 0;

  int		    paused = FALSE;

  memset( &sp, 0, sizeof( sp ) ); // это наверное по точкам
  memset( &sd, 0, sizeof( sd ) ); // это наверное по дисплею?
  ...
  init_stars (num_test, prev, cur, m, vk, ak, max_stars, &sp, &sd, DEBUG_var);
  ...
  while (TRUE)
  {
    ...
    if (my_count_max > 0) // -1 , например, будет означать бесконечный цикл
      if (my_count >= my_count_max) {
        //stop = TRUE;
        break; // жесткий выход. без обиняков
      }
    ...
    temp_points = prev;
    prev  = cur;
    cur   = temp_points; // меняем местами 
    ...
    // по умолчанию: move_fna = move_stars_ab7
    // 
    ...
    {
      new_star (num_test, prev, cur, m, vk, ak, max_stars, &sp, &sd, 0, DEBUG_var);

      continue;
    }
	
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // попробуем распечатать здесь, что имеем

    int ind;

    printf (/* stderr, */ "~~ my_count = %d ~~~~ sd.raw_num_steps = %d ~~~~~~~~~~~~~ \n", 
            my_count, sd.raw_num_steps);

    for( ind = 0; ind < max_stars; ind++ ) {

      printf (/* stderr, */ "ind= %4d   x= %12f  y= %12f \n", 
              ind, cur[ind].x, cur[ind].y);
    }
      ...
      do
      {
        ...
        if( add || add_star )
        {
          ...
          if( found != -1 )
          {
            ...
            new_star (num_test, prev, cur, m, vk, ak, max_stars, &sp, &sd,
                      add , DEBUG_var);
          }
        ...
        if (stop)
        {
          ...
          if ( verbose > 0 && last_init != 0 )
          {
            ...
            if ( verbose == 1 )
              ...
              fprintf ( stderr, 
                      "%s:%d  total life span of the star system: %ld sec (%d steps)\n", 
