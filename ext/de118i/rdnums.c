...
extern DOUBLE yn0[], yn1[], GMs[], Je[], Jm[], Cnm[], Snm[]; // указатели?
              // а где сама память то выедляется ??? 
...
struct number nums[] = {
  ...
  {yn0,      6*(NBODY+FMASS)}, // сюда же входят Lunar librations (Euler angles)
...
rdnums ()
{
  ...
  pnum = nums; // указатель на начало массива структур описания данных

inloop:

  px = pnum->val; // указатель на массив, куда будем считывать данные
  n  = pnum->n;   // сколько данных будем считывать
  ...
  for (i = 0; i < n; i++ )  // считываем, указанный набор
  {
  ...
  ++pnum; // встаем на след. позицию в табличке
