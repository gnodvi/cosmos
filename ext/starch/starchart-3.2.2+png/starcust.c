...
static char rcsid[]="$Header: /cvsroot/starchart/starchart/starcust.c,v 1.1.1.1 2000/07/27 19:00:12 gfk Exp $";
...
*/
  ...
  for (i = 0; i < numwins; i++) {
    ...
    if (!(mapwin[n] = (mapwindow *) malloc(sizeof(mapwindow))))
      die("Out of memory","");
    if (!(mapwin[n]->tag_field = (char *) malloc(MAXPATHLEN)))
      die("Out of memory","");
    ...
    for (j = 0; j < mapwin[n]->numfiles; j++) {
      ...
      if (!(mapwin[n]->file[o].name = (char *) malloc(MAXPATHLEN)))
	die("Out of memory","");
