...
static char rcsid[]="$Header: /cvsroot/starchart/starchart/starm2.c,v 1.1.1.1 2000/07/27 19:00:12 gfk Exp $";
     ...
     mapwindow *win;
{
  ...
  if(win->ra_step < 0 ) win->ra_step = - win->ra_step;
  ...
  for (glonc = 0.0; glonc < 360.0; glonc += win->ra_step) {
    ...
    olon = glon;
    for (glatc = -90.0 + ANGSKIP; glatc <= 90.0; glatc += ANGSKIP) {
      if (clipr_xform(glatc - ANGSKIP, olon, glatc, glon,
     ...
     mapwindow *win;
{
  ...
  if (win->dec_step < 0) win->dec_step = - win->dec_step;

  glatc = win->dec_strt;
  /* search down to find southernmost line */
  while ((olat =  glatc - win->dec_step) > -90.0) glatc = olat; 

  while (glatc < 90.0) {
    for (glonc = ANGSKIP; glonc <= 360.0; glonc += ANGSKIP) {
      if (clipr_xform(glatc, glonc - ANGSKIP, glatc, glonc,
